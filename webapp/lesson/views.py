...
def download():
    ...
    if current_user.is_admin:
        ...
        access_right_exists = Access_rights.query.filter(Access_rights.user_id==current_user.id,
                                                        Access_rights.category_id==category_id,
                                                        Access_rights.course_id==course_id,
                                                        Access_rights.grant_access==True).first()
    if access_right_exists:  
        my_bucket = get_bucket_from_s3()
        if my_bucket != None:
            file_obj = my_bucket.Object(file_to_download).get()
        return Response(
            file_obj['Body'].read(),
            mimetype='text/plain',
            headers={"Content-Disposition": "attachment;filename={}".format(file_to_download)}
        ) 
...
def lesson_page(category_name, course_name, lesson_name):
    ...
    if category_exists:
        course_exists = Course.query.filter(Course.name==course_name, 
                                            Course.category_id==category_exists.id).first() 
        if course_exists:
            if current_user.is_admin:
                access_right_exists = True
            else:    
                access_right_exists = Access_rights.query.filter(Access_rights.user_id==current_user.id,
                                                                ...
                                                                Access_rights.grant_access==True).first()
            if access_right_exists:                                                 
                lesson_exists = Lesson.query.filter(Lesson.name==lesson_name, 
                                                    Lesson.course_id==course_exists.id).first()
                if lesson_exists:
                    if request.method == 'POST':
                        return add_lesson_content(lesson_exists, category_name, course_name, lesson_name)                              
                    elif request.method == 'GET':
                        return show_lesson_content(lesson_exists, category_name, course_name, lesson_name)
                    else:
...
def show_lesson_content(lesson_exists, category_name, course_name, lesson_name):
    title = lesson_name
    is_homepage = False
    is_loginpage = False
...
def add_lesson_content(lesson_exists, category_name, course_name, lesson_name):
...
def delete_a_lesson():
    if current_user.is_admin or current_user.is_teacher:
        ...
        course_id=lesson_for_delete.course.id
        category_name=lesson_for_delete.course.category.name
        category_id=lesson_for_delete.course.category.id
